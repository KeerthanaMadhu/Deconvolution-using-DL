...
def model_autoencoder_withBN_reg(lr = 0.001,loss_type = 'mean_squared_error',reg_lambda = 0.001):
    input_img = Input(shape=(128, 128, 1))  # adapt this if using `channels_first` image data format
    ...
    x = keras.layers.BatchNormalization()(x)
    x = UpSampling2D((2, 2))(x)
    decoded = Conv2D(1, (3, 3), activation='relu', padding='same',kernel_initializer=initializers.RandomNormal(stddev = 2/(3*3*16)),
            kernel_regularizer=regularizers.l2(reg_lambda))(x)
...
def model_cnn_BN(lr = 0.001,loss_type = 'mean_squared_error',reg_lambda = 0.001):
    input_img = Input(shape=(128, 128, 1))  # adapt this if using `channels_first` image data format

    x = Conv2D(16, (7, 7), activation='relu', padding='same',kernel_initializer=initializers.RandomNormal(stddev = 2/(7*7*1)),
            kernel_regularizer=regularizers.l2(reg_lambda))(input_img)
    x = keras.layers.BatchNormalization()(x)
    x = Conv2D(16, (5, 5), activation='relu', padding='same',kernel_initializer=initializers.RandomNormal(stddev = 2/(5*5*16)),
           kernel_regularizer=regularizers.l2(reg_lambda))(x)
    x = keras.layers.BatchNormalization()(x)
    x = Conv2D(16, (3, 3), activation='relu', padding='same',kernel_initializer=initializers.RandomNormal(stddev = 2/(3*3*16)),
           kernel_regularizer=regularizers.l2(reg_lambda))(x)
...
def model_cnn_BN_uniform(lr = 0.001,loss_type = 'mean_squared_error',reg_lambda = 0.001):
    input_img = Input(shape=(128, 128, 1))  # adapt this if using `channels_first` image data format
