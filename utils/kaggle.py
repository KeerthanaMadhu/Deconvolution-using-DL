...
def my_LeNet(input_x, input_y, is_training,
    ...
    conv_layer_0 = conv_layer(input_x=input_x,
                              ...
                              rand_seed=seed)
    normlayer_0 = norm_layer(input_x = conv_layer_0.output(), is_training = tf.constant(True))
    pooling_layer_0 = max_pooling_layer(input_x=normlayer_0.output(),
    ...
    normlayer_1 = norm_layer(input_x = conv_layer_1.output(), is_training = tf.constant(True))
    pooling_layer_1 = max_pooling_layer(input_x=normlayer_1.output(),
    ...
    #dropout0 = tf.layers.dropout(
     # inputs=fc_layer_0.output(), rate=0.7, training=tf.constant(True))
    #normlayer_1 = norm_layer(input_x = fc_layer_0.output(), is_training = tf.constant(True))
...
def my_training(X_train, y_train, X_val, y_val,
    ...
    with tf.name_scope('inputs'):
        xs = tf.placeholder(shape=[None, res, res, 3], dtype=tf.float32)
...
def my_training_task4(X_train, y_train, X_val, y_val,
    ...
    # augment data
    Generator2 = ImageGenerator(x = X_train, y = y_train)
    translate = Generator2.translate(5,8)
    Generator2.x=X_train
    Generator2.y=y_train
    flipped = Generator2.flip(mode = 'h')
    '''
    Generator2.x=X_train
    Generator2.y=y_train
    added = Generator2.add_noise(portion = 0.1, amplitude = 5)
    '''
    Generator2.create_aug_data()
    X_aug_train = Generator2.x_aug
    y_aug_train = Generator2.y_aug
    
    # normalising
    X_aug_train = X_aug_train.reshape((X_aug_train.shape[0],X_aug_train.shape[1]*X_aug_train.shape[2]*X_aug_train.shape[3]))
    X_val = X_val.reshape((X_val.shape[0],X_val.shape[1]*X_val.shape[2]*X_val.shape[3]))
    mean_image = np.mean(X_aug_train, axis=0)
    X_aug_train = X_aug_train.astype(np.float32) - mean_image.astype(np.float32)
    X_val = X_val.astype(np.float32) - mean_image
    X_aug_train= X_aug_train.reshape([-1,32,32,3]) / 255
    X_val = X_val.reshape([-1,32,32,3]) / 255
    
    Generator2.x = X_aug_train
    Generator2.y = y_aug_train
    
    iters = int(Generator2.x.shape[0] / batch_size)
    print('number of batches for training: {}'.format(iters))
    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            for itr in range(iters):
                ...
                gen = Generator2.next_batch_gen(batch_size,shuffle = True)
                training_batch_x, training_batch_y = next(gen)
