    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                x, cache[cache_name] = bn_forward(x, gamma, beta, bn_params[i], mode = "train") 
            ...
            if dropout_config['enabled']:
                cache_name = "do_{}".format(i)
                x,cache[cache_name] = dropout_forward(x, dropout_config, mode="train")
                #pass
            ###############################################
            # END OF DROPOUT                               #
            ###############################################
        ...
        for i in range(num_layers - 1):
            ...
            ###############################################
            if dropout_config["enabled"]:
                dx = dropout_backward(dx,cache["do_{}".format(j)])
                #pass
            ###############################################
            # END OF DROPOUT                               #
            ###############################################     
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                gamma = params["bn_gamma_{}".format(i)]
                beta = params["bn_beta_{}".format(i)]
                x, _ = bn_forward(x, gamma, beta, bn_params[i], mode = "test")
